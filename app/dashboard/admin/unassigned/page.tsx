...
export default function UnassignedPage() {
  ...
  const assignOne = async (ticketId: string, agentId: string) => {
    ...
    await supabase.from('activity_log').insert({
      action: 'ticket.assigned', target_type: 'ticket', target_id: ticketId,
  ...
  const autoAssign = async () => {
    ...
    for (const ticket of tickets) {
      ...
      if (!error) {
        await supabase.from('activity_log').insert({
          action: 'ticket.auto_assigned', target_type: 'ticket', target_id: ticket.id,
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th>#</th><th>Título</th><th>Prioridad</th><th>Categoría</th>
                <th>Solicitante</th><th>Espera</th><th>Asignar a</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={ticket.id}>
                    <td>
                      <input type="checkbox" checked={selected.includes(ticket.id)}
                        onChange={() => toggleSelect(ticket.id)} className="accent-blue-500 cursor-pointer" />
                    </td>
                    ...
                    <td><span className="pill text-[10px]" style={{ background: pr?.bg, color: pr?.color }}>{pr?.label}</span></td>
                    <td className="text-[12px]" style={{ color: '#4a5f76' }}>{cat?.icon} {cat?.label}</td>
                    ...
                    <td>
                      <div className="flex items-center gap-1.5">
                        <select value={assignMap[ticket.id] || ''}
                        ...
                        <button onClick={() => assignOne(ticket.id, assignMap[ticket.id])}
...
**5.** Verifica antes de guardar que la **línea 12** diga:
```
const [tickets, setTickets] = useState<any[]>([])
