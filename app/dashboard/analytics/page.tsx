...
export default async function AnalyticsPage() {
  ...
  const { data: tickets } = await supabase.from('tickets').select('status, priority, category, created_at, resolved_at, sla_breached, assigned_to')
  const { data: agentCount } = await supabase.from('profiles').select('id', { count:'exact', head:true }).in('role',['agent','admin'])
  ...
  let resolutionMs = 0; let resolvedN = 0
  ...
  const avgHours = resolvedN ? (resolutionMs / resolvedN / 36e5).toFixed(1) : '—'
  const slaBreached = tickets?.filter(t => t.sla_breached).length || 0
  const resolutionRate = total ? Math.round(((statusC.resolved||0) + (statusC.closed||0)) / total * 100) : 0
  ...
  const kpis = [
    ...
    { label: 'Agentes activos',    value: agentCount || 0,  icon: Users,     color: '#eab308' },
  ]
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 anim-fade">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        {/* Status */}
        <div className="panel">
          <div className="panel-header"><span className="text-[13px] font-semibold text-white">Por estado</span></div>
          <div className="p-4 space-y-3">
            {(Object.keys(STATUS_MAP) as TicketStatus[]).map(s => {
              const cfg = STATUS_MAP[s]; const n = statusC[s] || 0; const pct = total ? Math.round(n/total*100) : 0
              return (
                <div key={s}>
                  <div className="flex justify-between mb-1.5">
                    <span className="text-[12px]" style={{ color: '#7a8fa8' }}>{cfg.label}</span>
                    <span className="mono text-[11px]" style={{ color: cfg.color }}>{n} <span style={{ color:'#3d4e62' }}>({pct}%)</span></span>
                  </div>
        ...
        {/* Priority */}
        <div className="panel">
          <div className="panel-header"><span className="text-[13px] font-semibold text-white">Por prioridad</span></div>
          <div className="p-4 space-y-3">
            {(Object.keys(PRIORITY_MAP) as TicketPriority[]).map(p => {
              const cfg = PRIORITY_MAP[p]; const n = prioC[p] || 0; const pct = total ? Math.round(n/total*100) : 0
              return (
                <div key={p}>
                  <div className="flex justify-between mb-1.5">
                    <span className="text-[12px]" style={{ color: '#7a8fa8' }}>{cfg.label}</span>
        ...
        {/* Category */}
        <div className="panel">
          <div className="panel-header"><span className="text-[13px] font-semibold text-white">Por categoría</span></div>
          <div className="p-4 space-y-2">
            {Object.entries(catC).sort(([,a],[,b]) => b-a).map(([cat, n]) => {
              const cfg = CATEGORY_MAP[cat as TicketCategory]; const pct = total ? Math.round(n/total*100) : 0
              return (
                <div key={cat} className="flex items-center justify-between py-0.5">
                  <span className="text-[12px]" style={{ color: '#5a6f88' }}>{cfg?.icon} {cfg?.label}</span>
